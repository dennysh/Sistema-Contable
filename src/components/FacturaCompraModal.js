...
const FacturaCompraModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/facturas-compra', data);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-screen overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold">Nueva Factura de Compra</h2>
          <button
            onClick={onClose}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (articulo) {
                          handleDetalleChange(index, 'precio_unitario', articulo.precio_compra || 0);
                        }
                      }}
                      className="form-input"
                      required
                    >
                      ...
                      {articulos.map(articulo => (
                        <option key={articulo.id} value={articulo.id}>
                          {articulo.nombre} - ${articulo.precio_compra || 0}
                        </option>
                      ))}
                    </select>
                  </div>
...
export default FacturaCompraModal;
